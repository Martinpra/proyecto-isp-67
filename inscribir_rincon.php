...
$query=mysqli_query($datos_db, "SELECT * FROM insc_rincon WHERE dni='$dni_formu'") or die("error");

$row=mysqli_num_rows($query);

if($row > 0){
    header('location: rincon.php?error');

}

    else{
      mysqli_query($datos_db, "INSERT INTO insc_rincon VALUES (default, '$fecha_hora', '$dni_formu', '$apellido_formu','$nombre_formu', '$direccion_formu', '$correo_formu', '$telefono_formu', $edad_formu, '$fechanac_formu', '$localidad_formu', '$observaciones_formu')");
     ...
     mysqli_close($datos_db);
            ...
            header('location: rincon.php?id');
            header('location: rincon.php?ok');
        };
